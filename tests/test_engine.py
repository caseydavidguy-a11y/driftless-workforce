...
class EngineTests(unittest.TestCase):
    ...
    def test_groups_same_employer(self):
        jobs = [
            JobObservation("Acme, Inc.", "Operator", "La Crosse, WI", "manufacturing"),
            JobObservation("ACME LLC", "Supervisor", "La Crosse, WI", "leadership"),
        ]
        employers = build_employers(jobs)
        self.assertEqual(len(employers), 1)
        self.assertEqual(employers[0].opening_count, 2)
    ...
    def test_priority_is_assigned(self):
        ...
        self.assertIn(employer.priority, {"Pursue", "Monitor", "Low"})
        self.assertGreaterEqual(employer.score, 0)
        self.assertLessEqual(employer.score, 100)
