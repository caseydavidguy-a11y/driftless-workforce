...
def employer_from_snapshot(item: dict) -> Employer:
    ...
    for raw in item.get("observations",[]):
        row=dict(raw); row["posted_at"]=_parse_dt(row.get("posted_at")); observations.append(JobObservation(**row))
    employer=Employer(name=item["name"],canonical_name=item.get("slug",item["name"]))
    employer.locations.update(item.get("locations",[])); employer.industries.update(item.get("industries",[])); employer.observations.extend(observations)
    return employer
...
def compare_snapshots(previous: dict, current: dict) -> dict:
    ...
    for slug,employer in new.items():
        if slug not in old:
            signals=[HiringSignal("new_employer","high","New employer detected in the hiring feed", "new")]
        else:
            signals=compare_employer(employer,old[slug])
        if signals:report.append({"employer":employer.name,"slug":slug,"signals":[s.__dict__ for s in signals]})
