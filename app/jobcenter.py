...
def _split_combined_first_cell(text):
    ...
    if match:
        employer=match.group(1).strip(); title=before_source[:match.start()].strip()
...
def parse_results(html,source_url,requested_city):
    ...
    for row in parser.rows:
        ...
        if len(row)>=4:
            title,location,date_posted,employer=row[:4]
            employer=employer.split("Source:",1)[0].strip()
        else:
            title,employer=_split_combined_first_cell(row[0]); location,date_posted=row[1:3]
