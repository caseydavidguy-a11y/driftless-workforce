...
DATA=ROOT/"data"
REQUIRED_OPPORTUNITY={"employer","slug","score","priority","opening_count","verified_opening_count","locations","industries","score_breakdown","score_policy_version"}
REQUIRED_JOB={"employer","title","location","industry","source","source_url","external_id","verified"}
...
def validate():
    ...
    history=load("snapshot_history.json")
    snapshots=history.get("snapshots",history) if isinstance(history,(dict,list)) else []
    ...
    print(f"Validated {len(opportunities)} opportunities and {len(snapshots)} snapshots.")
